...
model = VSSM(in_chans=3,num_classes=4,depths=[2, 2, 2, 2],depths_decoder=[2, 2, 2, 2],drop_path_rate=0.2)

class main_model(nn.Module):

    def __init__(self, num_classes, in_chans=3, embed_dim=96, depths=(2, 2, 2, 2),
                 norm_layer=nn.LayerNorm, patch_norm=True, HFF_dp=0.,
                 conv_depths=(2, 2, 2, 2), conv_dims=(96, 192, 384, 768), conv_drop_path_rate=0.,
                 conv_head_init_scale: float = 1.,act_layer=nn.GELU, **kwargs):
        super().__init__()
        ...
        for i in range(4):
            stage = nn.Sequential(
                *[ConvolutionalGLU(dim=conv_dims[i], drop_rate=dp_rates[cur + j])
        ...
        self.fu1 = ACF(ch_1=96, ch_2=96, r_2=16, ch_int=96, ch_out=96, drop_rate=HFF_dp)
        self.fu2 = ACF(ch_1=192, ch_2=192, r_2=16, ch_int=192, ch_out=192, drop_rate=HFF_dp)
        self.fu3 = ACF(ch_1=384, ch_2=384, r_2=16, ch_int=384, ch_out=384, drop_rate=HFF_dp)
        self.fu4 = ACF(ch_1=768, ch_2=768, r_2=16, ch_int=768, ch_out=768, drop_rate=HFF_dp)
    ...
    def forward(self, imgs):
        ...
        ######  Global Branch ######
        # rand_in = torch.randn(1, 3, 224, 224).cuda()
        # x, skip_list = self.model(rand_in)
        x, skip_list = self.model(imgs)
        x_s_1 = skip_list[0].permute(0, 3, 1, 2)
        x_s_2 = skip_list[1].permute(0, 3, 1, 2)
        x_s_3 = skip_list[2].permute(0, 3, 1, 2)
        x_s_4 = skip_list[3].permute(0, 3, 1, 2)
...
class ConvolutionalGLU(nn.Module):
    def __init__(self, in_features, hidden_features=None,  act_layer=nn.GELU, drop=0.):
        ...
        self.dwconv = nn.Conv2d(hidden_features * 2, hidden_features*2, kernel_size=3, stride=1, padding=1, bias=True, groups=hidden_features*2)
        self.norm = LayerNorm(hidden_features*2, eps=1e-6, data_format="channels_last")
        self.act = act_layer()
